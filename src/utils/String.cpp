...
namespace std {
    size_t hash<r3::String>::operator()(const r3::String& str) const {
...
namespace r3 {
    ...
    String::String(const gjs::script_string& str) : m_data(nullptr), m_len(u32(str.length())), m_capacity(m_len) {
        m_data = (char*)Allocator::Get()->alloc(m_capacity + u64(1));
        memcpy(m_data, str.c_str(), str.length());
        m_data[m_len] = 0;
    }
